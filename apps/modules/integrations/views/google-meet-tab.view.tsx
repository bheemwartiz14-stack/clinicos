    ...
    <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="flex gap-3">
          ...
          <div>
            <h2 className="text-lg font-semibold text-foreground">Google Meet</h2>
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Generate Google Meet links for online consultations using Google Calendar conference data.</p>
          </div>
        </div>
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {canManage && integration?.status !== "connected" ? (
          <a href="/api/integrations/google-meet/connect" className="inline-flex h-10 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
            <ExternalLink className="h-4 w-4" aria-hidden /> Connect Google Meet
          </a>
        ) : null}
        {canManage && integration?.status === "connected" ? (
          <>
            <button type="button" onClick={() => post("test")} disabled={pending !== null} className="inline-flex h-10 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:opacity-60">
              <TestTube2 className="h-4 w-4" aria-hidden /> Test Google Meet Link
            </button>
            <button type="button" onClick={saveSettings} disabled={pending !== null} className="inline-flex h-10 items-center gap-2 rounded-xl border border-border bg-background px-4 text-sm font-semibold text-foreground hover:bg-muted disabled:opacity-60">
              <Save className="h-4 w-4" aria-hidden /> {pending === "save" ? "Saving..." : "Save Meet Settings"}
            </button>
            <button type="button" onClick={() => post("disconnect")} disabled={pending !== null} className="inline-flex h-10 items-center gap-2 rounded-xl border border-destructive/20 bg-destructive/10 px-4 text-sm font-semibold text-destructive hover:bg-destructive/20 disabled:opacity-60">
