    ...
    <div className="mt-5 rounded-xl border border-teal-100 bg-teal-50/70 p-4">
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`inline-flex h-10 items-center gap-2 rounded-lg px-3 text-sm font-semibold transition ${
              activeTab === tab.id ? "bg-teal-700 text-white" : "bg-white text-teal-800 ring-1 ring-teal-100 hover:bg-teal-50"
            }`}
          >
      ...
      {activeTab === "schedule" ? (
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <p className="rounded-lg border border-teal-100 bg-white p-3 text-sm text-teal-800 md:col-span-2">Schedule tabs are ready for the doctor-owned schedule tables. These controls are not assigned to admin branch or department management.</p>
        </div>
      ) : null}
      ...
      {activeTab === "availability" ? (
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <p className="rounded-lg border border-teal-100 bg-white p-3 text-sm text-teal-800 md:col-span-2">Doctors own availability slots; admins should only see resulting capacity for booking and operations.</p>
        </div>
      ) : null}
      ...
      {activeTab === "leave" ? (
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          ...
          <p className="rounded-lg border border-teal-100 bg-white p-3 text-sm text-teal-800 md:col-span-2">Leave and block dates belong to the doctor workflow and should block appointment slot recommendations.</p>
        </div>
      ) : null}
        ...
        <div className="mt-4 rounded-lg border border-teal-100 bg-white p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-600">Connect OAuth here when the integration keys and callback route are ready.</p>
            </div>
            <button type="button" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-teal-200 px-4 text-sm font-semibold text-teal-800">
      ...
      {activeTab === "ai" ? (
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <div className="rounded-lg border border-teal-100 bg-white p-4">
            <p className="font-semibold text-slate-900">AI schedule optimization</p>
            <p className="mt-2 text-sm text-slate-600">Use appointment history, leaves, and preferred hours to reduce gaps.</p>
          </div>
          <div className="rounded-lg border border-teal-100 bg-white p-4">
            <p className="font-semibold text-slate-900">AI slot recommendation</p>
            <p className="mt-2 text-sm text-slate-600">Recommend the best slot from doctor availability and patient urgency.</p>
          </div>
        </div>
      ) : null}
    </div>
...
export function ProfileForm({ profile, branches, departments }: { profile: SettingsProfile; branches: SettingsOption[]; departments: SettingsOption[] }) {
  ...
  return (
    <form action={action} className="grid gap-5">
      <section id={isDoctor ? "doctor-practice" : undefined} className="rounded-xl border border-white/70 bg-white/80 p-5 shadow-lg shadow-teal-950/5 backdrop-blur">
        <div className="mb-5">
          <h2 className="text-lg font-semibold text-slate-950">Profile Information</h2>
          <p className="mt-1 text-sm text-slate-500">Keep identity, contact, assignment, and professional details current.</p>
        </div>
      ...
      <section className="rounded-xl border border-white/70 bg-white/80 p-5 shadow-lg shadow-teal-950/5 backdrop-blur">
        <h2 className="text-lg font-semibold text-slate-950">Contact & Address</h2>
        <div className="mt-5 grid gap-4 md:grid-cols-2">
      ...
      <section className="rounded-xl border border-white/70 bg-white/80 p-5 shadow-lg shadow-teal-950/5 backdrop-blur">
        ...
        {isDoctor ? (
          <>
            <div className="mt-5 rounded-lg border border-teal-100 bg-teal-50 px-4 py-3 text-sm text-teal-800">
      ...
      <div className="sticky bottom-4 z-10 flex justify-end">
        <button
          disabled={pending}
          className="inline-flex items-center gap-2 rounded-lg bg-teal-700 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-teal-900/20 transition hover:bg-teal-800 disabled:cursor-not-allowed disabled:opacity-60"
        >
